...
function removeReservedTicketStatus(ticketId) {
    $.ajax({
        type: "POST",
        url: 'masterInterface.php',
        data: {func: "AJAXRemoveReservedStatus", arg1:ticketId},
        success: function(data) {
             if(data === "no"){
                console.log("ticket status no actualizado");
             } else {
                    console.log(data);
                    console.log("ticket status actualizado");
            }
        }
    });
}
...
function hasDigits(num) {
    var tickets = dbTickets;
    var selectedNumbers = [];
    var selectedTicketNumbers = [];
    var number = num.toString();
    
    for (i = 0; i < tickets.length; i++) {
        for (j = 0; j < number.length; j++) {
            console.log($.inArray(tickets[i].TicketNumber, selectedTicketNumbers) == -1);
            if ($.inArray(number[j], tickets[i].TicketNumber) != -1) {
                if($.inArray(tickets[i].TicketNumber, selectedTicketNumbers) == -1) {
                    selectedNumbers.push(tickets[i]);
                    selectedTicketNumbers.push(tickets[i].TicketNumber);
                }
            }
        }
    }
...
function clearCartList(){
    console.log('clearCartList called');
